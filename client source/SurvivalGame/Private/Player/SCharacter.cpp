...
void ASCharacter::Use()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		{
			Usable->OnUsed(this);
			ASPickupActor* ITEM = Cast<ASPickupActor>(Usable);
			Item_info.IsUsed[ITEM->ItemIndex] = true;
			TActorIterator<AItem_Manager> It(GetWorld());
			auto item_manager = *It; 
			auto ItemArray = item_manager->GetItemArray();
			(*ItemArray)[ITEM->ItemIndex] = nullptr;
		}
	}
	else
	{
